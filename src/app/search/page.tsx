...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<SearchPageParams> }) {
  ...
  if (!query) return <PageShell className="space-y-8 pb-32 pt-4"><SearchHeader sort={sort} title="Search" /><section className="space-y-4 pt-3" aria-live="polite"><div className="space-y-2"><h2 className="text-lg font-semibold text-white">Search for a movie or TV show.</h2><p className="text-sm text-white/60">Type a title to begin.</p></div><div className="flex flex-wrap items-center gap-3"><p className="text-sm text-white/60">Not sure what to search for?</p><Link href="/discover" className="inline-flex h-10 items-center border border-white/10 bg-white px-4 text-sm font-semibold text-ink-950 transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember-300">Explore Movies</Link></div></section></PageShell>;
